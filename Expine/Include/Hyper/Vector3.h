...
namespace Hyper
{
	...
	public:
		...
		static FORCEINLINE Float DistSquaredXY
		(
			const Vector3f & V1, 
			const Vector3f & V2
		);
		...
		(
					TVector<Vector3f> & Clusters, 
			const	TVector<Vector3f> & Points, 
...
#define INTEL_ORDER_VECTOR(x) (x)
	...
	FORCEINLINE Vector3f Vector3f::GetSafeNormal2D(Float Tolerance) const
	{
		...
		const Float Scale = Math::InvSqrt(SquareSum);
		return Vector3f(X*Scale, Y*Scale, 0.f);
	}
	...
	{
		return Math::Sqrt(Vector3f::DistSquaredXY(V1, V2));
	}
	...
	FORCEINLINE Float Vector3f::DistSquaredXY
	(
		const Vector3f & V1, 
		const Vector3f & V2
	)
